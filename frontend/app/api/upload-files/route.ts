...
export async function POST(request: NextRequest) {
  try {
    console.log('🔍 Frontend file upload request received')
    ...
    const uploadUrl = `${backendUrl}/api/upload-files`
    ...
    if (!backendResponse.ok) {
      ...
      console.error('❌ Backend file upload failed:', errorData)
    ...
    console.log('✅ Backend file upload successful:', responseData)

    return NextResponse.json(responseData)

  } catch (error) {
    console.error('❌ Frontend file upload endpoint error:', error)
